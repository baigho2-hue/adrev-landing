...
<<<<<<< HEAD
                ...
                {
                    AddPageBreak(body);
                    AddHeading(body, "ANNEXES");
                    foreach (var app in protocol.Appendices)
                    {
                        AddParagraph(body, $"ANNEXE : {app.Title} ({app.Type})", true);
...
<<<<<<< HEAD
        public void ExportAppendicesOnly(List<ProtocolAppendix> appendices, string projectTitle, string filePath)
        {
            CreateWordDocument(filePath, (body) =>
            {
                AddMainTitle(body, "DOSSIER DES ANNEXES");
                AddParagraph(body, $"Projet : {projectTitle}", true);
                AddParagraph(body, "--------------------------------------------------");

                if (appendices == null || !appendices.Any())
                {
                    AddParagraph(body, "Aucune annexe n'a été ajoutée.");
                    return;
                }

                foreach (var (app, index) in appendices.Select((a, i) => (a, i + 1)))
                {
                    AddHeading(body, $"Annexe {index} : {app.Title}");
                    AddParagraph(body, $"Type : {app.Type}", true);
                    
                    if (!string.IsNullOrEmpty(app.Content))
                    {
                        AddParagraph(body, app.Content);
                    }
                    ...
                    {
                        AddParagraph(body, $"Fichier joint : {Path.GetFileName(app.FilePath)}", true);
                        AddParagraph(body, "(Veuillez consulter le fichier original dans le dossier du projet)");
                    }
                    
                    if (index < appendices.Count)
                    {
                        AddPageBreak(body);
                    }
                }
